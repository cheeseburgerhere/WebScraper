...
def __webPull(link:list,tags:list, split:bool=True): #pulls data fram website
...
def __list_to_df(input:dict, sites : dict, reqDate:datetime.datetime, num): #creates the dataframe
    names=[]
    dates=[]
    siteN=[]
    for obj in input:
...
def contentsPuller(df:pd.DataFrame): #gets the article content
    siteLinks:list=df.iloc[:,2]

    pull=__webPull(link=siteLinks,tags=["div","p"], split=False)
    content = []
    for c in pull:
